...
# changing invaders
# by david
# combine t-SNE created in one plot
# cut -d, -f7-14 SNP.csv|sed '1s/_[^,]*//g'|tr , \\t|awk '{print $1"\t"$4"\t"$2"\t"$3"\t"$5"\t"$6"\t"$7"\t"$8}'
# ggplot for plotting of the t-SNE
library(ggplot2)
...
for (genotypes in rev(process)) {
	...
	# plot the positions
	created.plot <- ggplot(dim.2, aes(V1, V2)) + geom_point() + geom_label_repel(aes(label = sample, fill = Area)) +
		xlab("t-SNE dimension 1") + ylab("t-SNE dimension 2") + scale_fill_manual(breaks = c("Island", "Semi-Mainland", "Mainland"), values=c("Blue", "Yellow", "darkGreen")) + theme(legend.position = "none")
	if (first.run) {
