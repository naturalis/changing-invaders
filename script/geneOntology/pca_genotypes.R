...
if (answer != 0) {
  answer <- list.files(pattern = "*.bgt")[answer]
	...
	ggplot(dim.2, aes(PC1, PC2)) + geom_point() + geom_label_repel(aes(label = sample)) +
	  ...
	  ggtitle(paste("Principial Correspondence Analysis over all", sub("\\.bgt$", "", answer), " coding SNPs"), subtitle = "Discarding all sysnonymous coding SNPs")
	# awk '$4==$6&&$4!=$1&&$4!=$2&&$4!=$3&&$4!=$5&&$4!=$7&&$4!=$8{print}' ASM.gt
  ggsave(sub("\\.bgt$", "_pca.png", answer))
  save(genotypes, file = sub("\\.bgt$", ".pca", answer))
}
