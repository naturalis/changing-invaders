...
# table(how_many_hits)
# if they are all true, then nothing should be removed anyway,
# however due the function it would remove everything
if(!all(how_many_hits)) json_combination <- remove_this(json_combination, !how_many_hits)
...
# table(contains_gap)
# same applies here
if(!all(!contains_gap)) json_combination <- remove_this(json_combination, contains_gap)
