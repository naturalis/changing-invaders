...
if (file.exists("~/structure/aus_map.rdata")) load("~/structure/aus_map.rdata") else {
	...
	save(aus_map, file = "~/structure/aus_map.rdata")
}
for (Q.file in Q.files) {
	tbl_orig <- read.table(Q.file)
	tbl <- round(tbl_orig, digits = 2)
	rownames(tbl) <- sub("_.*", "", read.table("~/structure/merge8.fam")$V2)
	colnames(tbl) <- as.character(1:ncol(tbl))
	# the sample name in the plot is the sort (up to the _) version within the fam file
	tbl$SAMPLE <- sub("_.*", "", read.table("~/structure/merge8.fam")$V2)
	sub("(([1-9]+)[0-9]?\\b)", "(assumption: \\1 anchestors)", sub("(admixt|struct)", "\\1ure", gsub("\\.|-", " ", sub("\\.(mean)?Q$", "", Q.file))))
	nanchestors <- as.numeric(rev(strsplit(Q.file, "\\.")[[1]])[2])

	# create a long table with the groups
	better_table <- full_join(tabel, tidyr::pivot_longer(tbl, c(as.character(1:(ncol(tbl) - 1))), "group", values_to = "chance"), "SAMPLE")
	# store the most apparent group
	table_v <- full_join(tabel, better_table %>% group_by(SAMPLE) %>% summarise(group = group[chance == max(chance)]), "SAMPLE")
