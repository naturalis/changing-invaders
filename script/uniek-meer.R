...
zoekterm <- exulans %>% filter(DIST_N > 299L, DIST_P > 299L) %>% group_by(chromosome, position) %>% summarise(heterozygoot = !(n_distinct(paste0(GENOTYPE, "+", ALTERNATIVE)) == 1L && count(GENOTYPE) == 8L), p = COUNT(REFERENCE), COVERAGE_THRESHOLD = COVERAGE > 16L && 110L > COVERAGE, QUALITY_THESHOLD = QUALITY > 99L) %>% filter(heterozygoot, COVERAGE_THRESHOLD, QUALITY_THESHOLD)
